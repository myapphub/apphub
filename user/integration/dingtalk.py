...
class DingtalkAccount(ProviderAccount):
    ...
    def to_str(self):
        return self.account.extra_data.get("nick", super(DingtalkAccount, self).to_str())
...
class CustomDingTalkProvider(OAuth2Provider):
    ...
    def extract_common_fields(self, data):
        ...
        return dict(username=data.get("nick"), name=data.get("nick"), first_name=first_name, last_name=last_name, email=data.get("email", None))
...
class DingtalkOAuth2Client(OAuth2Client):
    ...
    def get_access_token(self, code):
        ...
        if (
            not access_token
            or "accessToken" not in access_token
        ):
